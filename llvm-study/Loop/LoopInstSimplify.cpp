  ...
  for (;;) {
    ...
    for (BasicBlock *BB : RPOT) {
      for (Instruction &I : *BB) {
        LLVM_DEBUG(dbgs() << "Current : "; I.dump());
        ...
        if (I.use_empty()) {
          ...
          if (isInstructionTriviallyDead(&I, &TLI)) {
            LLVM_DEBUG(dbgs() << "isInstructionTrivially: ";I.dump());
    ...
    if (!DeadInsts.empty()) {
      for (Value *V : DeadInsts) {
        Instruction *I = cast_or_null<Instruction>(V);
        if (!I)
          continue;
        LLVM_DEBUG(dbgs() << "DeadInsts : "; I->dump());
      }
...
static RegisterPass<LoopInstSimplifyLegacyPass> X("loop-instsimplify-study", "Loop simplify Pass");

char LoopInstSimplifyLegacyPass::ID = 0;

INITIALIZE_PASS_BEGIN(LoopInstSimplifyLegacyPass, "loop-instsimplify-study",
                      "Simplify instructions in loops", false, false)
INITIALIZE_PASS_DEPENDENCY(AssumptionCacheTracker)
INITIALIZE_PASS_DEPENDENCY(LoopPass)
INITIALIZE_PASS_DEPENDENCY(MemorySSAWrapperPass)
INITIALIZE_PASS_DEPENDENCY(TargetLibraryInfoWrapperPass)
INITIALIZE_PASS_END(LoopInstSimplifyLegacyPass, "loop-instsimplify-study",
                    "Simplify instructions in loops", false, false)
